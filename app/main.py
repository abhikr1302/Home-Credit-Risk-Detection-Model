...
def get_expected_features(
    ...
    raise ValueError(
        ...
        "the original input feature names."
    )
...
app = FastAPI(
    ...
    description=(
        "Predicts the probability that an applicant "
        "will experience repayment difficulty."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
def model_information(
    ...
    expected_features = (
        request.app.state.expected_features
    )
...
def predict(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "Prediction failed. Check the feature "
                f"values and data types. Error: {error}"
            ),
        ) from error
